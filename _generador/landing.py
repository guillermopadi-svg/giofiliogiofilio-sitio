...
# ------------------------------------------------------------------ HOME
def track_attrs(l, event, source):
    """Atributos data-* que lee assets/js/home.js para armar el evento."""
    return (f'data-track="{event}" data-source="{e(source)}" data-prop-id="{e(l["id"])}" '
            f'data-prop-name="{e(l["title"].replace(" — ", " "))}" data-prop-type="{e(l["propertyType"])}" '
            f'data-prop-loc="{e(l["locationLabel"])}"')


def _facts(l):
    return [
        ("area", f'{l["surface"]:,.2f} m²', "Superficie"),
        ("car", str(l["parkingSpaces"]), "Estacionamientos"),
        ("users", str(l["privateOffices"]), "Privados"),
        ("layers", f'Piso {l["floor"]}', "Ubicación"),
        ("cert", l["certification"], "Certificación"),
...
def home_hero(l, R):
    """Hero del home con la propiedad destacada (foto real, sin inventar datos)."""
    facts = "".join(f'<li>{_ic(ic)}<span><b>{e(v)}</b><small>{e(lab)}</small></span></li>' for ic, v, lab in _facts(l))
    jpg, webp = _imgs(l["heroImage"])
    name = l["title"].split(" — ")
    wa = f'https://wa.me/{MARCA["whatsapp"]}?text={quote(_wa_text(l))}'
    return f'''<section class="home-hero" data-reveal-off>
  <div class="home-hero-media"><picture><source type="image/webp" srcset="{R(webp)}">
    <img src="{R(jpg)}" alt="{e(l["gallery"][0][1])}" fetchpriority="high" width="948" height="948"></picture></div>
  <div class="wrap home-hero-inner">
    <p class="eyebrow hero-eyebrow">Propiedad destacada</p>
    <h1><span class="sr-only">Gio Filio, asesoría inmobiliaria en Ciudad de México: </span>{e(name[0])}<em>{e(name[1]) if len(name) > 1 else ""}</em></h1>
    <p class="home-hero-sub">{e(l["subtitle"])}</p>
    <ul class="home-facts">{facts}</ul>
    <div class="home-hero-cta">
      <a class="btn btn--gold-solid btn--lg" href="{R(landing_url(l))}" {track_attrs(l, "click_property_cta", "home_hero")}>Ver propiedad {icon("arrow")}</a>
      <a class="btn btn--outline-light btn--lg" href="{wa}" target="_blank" rel="noopener" {track_attrs(l, "click_property_whatsapp", "home_hero")}>{icon("wa")} Contactar por WhatsApp</a>
    </div>
  </div>
</section>'''


def home_featured(ls, path, R, card_grid):
    """Sección de propiedades destacadas: solo las que existen (featured=True).
    1 → composición editorial; 2 o más → tarjetas."""
    if len(ls) == 1:
        l = ls[0]
        jpg, webp = _imgs(l["homeImage"])
        cjpg, cwebp = _imgs(l["homeImage"], "card")
        facts = "".join(f'<li>{_ic(ic)}<span><b>{e(v)}</b><small>{e(lab)}</small></span></li>' for ic, v, lab in _facts(l))
        alt = next((a for b, a in l["gallery"] if b == l["homeImage"]), l["title"])
        url = R(landing_url(l))
        inner = f'''<article class="feat-editorial" data-reveal>
      <a class="feat-photo" href="{url}" aria-label="Ver {e(l["title"])}" {track_attrs(l, "click_featured_property", "home_featured_photo")}>
        <picture><source type="image/webp" srcset="{R(cwebp)} 640w, {R(webp)} 948w" sizes="(max-width:900px) 100vw, 62vw">
          <img src="{R(jpg)}" srcset="{R(cjpg)} 640w, {R(jpg)} 948w" sizes="(max-width:900px) 100vw, 62vw" alt="{e(alt)}" loading="lazy" decoding="async" width="948" height="948"></picture>
        <span class="badge badge--destacada">Destacada</span>
      </a>
      <div class="feat-info">
        <p class="feat-kicker">{e(l["propertyType"])} · {e(l["locationLabel"])}</p>
        <h3>{e(l["title"])}</h3>
        <p class="feat-sub">Oficina corporativa {e(l["buildingClass"])} con {e(l["certification"])}.</p>
        <ul class="home-facts home-facts--grid">{facts}</ul>
        <p class="feat-op">{e(OP_LABEL[l["operation"]])} · Precio a consultar</p>
        <a class="btn btn--lg btn--block feat-cta" href="{url}" {track_attrs(l, "click_property_cta", "home_featured")}>Ver propiedad {icon("arrow")}</a>
      </div>
    </article>'''
    else:
        inner = f'<div data-reveal data-home-cards>{card_grid(path, [shadow_prop(l) for l in ls])}</div>'
    return f'''<section class="section home-featured" id="destacadas">
  <div class="wrap">
    <div class="home-featured-head" data-reveal>
      <div>
        <p class="eyebrow">Propiedad destacada</p>
        <h2>{e(ls[0]["title"]) if len(ls) == 1 else "Propiedades destacadas"}</h2>
        <p class="lead">{"Oficina corporativa acondicionada en Santa Fe, Ciudad de México." if len(ls) == 1 else "Selección de Gio Filio."}</p>
      </div>
      <a class="link-arrow" href="{R("propiedades/")}">Ver todo el inventario{icon("arrow")}</a>
    </div>
    {inner}
  </div>
</section>'''
...
def home_sticky(l, R):
    """Barra inferior móvil del home: WhatsApp | Agendar visita (lleva a la landing)."""
    wa = f'https://wa.me/{MARCA["whatsapp"]}?text={quote(_wa_text(l))}'
    return (f'<div class="lp-sticky home-sticky" role="group" aria-label="Contactar sobre {e(l["title"])}">'
            f'<a class="btn btn--wa" href="{wa}" target="_blank" rel="noopener" {track_attrs(l, "click_property_whatsapp", "home_sticky")}>{icon("wa")} WhatsApp</a>'
            f'<a class="btn btn--gold-solid" href="{R(landing_url(l))}#agendar" {track_attrs(l, "click_property_cta", "home_sticky_agendar")}>Agendar visita</a></div>')
